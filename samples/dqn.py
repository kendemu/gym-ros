...
from keras.layers import Convolution2D, Flatten, Dense, InputLayer, Permute, Activation

KERAS_BACKEND = 'tensorflow'

ENV_NAME = 'ROSEnv-v0'  # Environment name
...
class Agent():
    ...
    def build_network(self):
        model = Sequential()
        #stack 4 frames of imaoges
        model.add(Permute((1,2,3), input_shape=((STATE_LENGTH,) + (FRAME_HEIGHT, FRAME_WIDTH))))
        ...
        s = tf.placeholder(tf.float32, [1, STATE_LENGTH, FRAME_WIDTH, FRAME_HEIGHT])
        q_values = model(s)
...
def main():
    ...
    h,w = env.action_space.shape
    n = h * w
    agent = Agent(num_actions=n)
